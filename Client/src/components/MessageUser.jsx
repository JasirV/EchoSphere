...
getUsers()
  ...
  useEffect(() => {
    const getMessages = async () => {
      try {
        const res = await axios.get(`https://echosphere-5ixt.onrender.com/message/${currentChat?._id}`);
  ...
  const receiverId = currentChat.members?.find(member=> member !==id)
    ...
    try {
      const data=await axios.post('https://echosphere-5ixt.onrender.com/message',{message});
  ...
  useEffect(()=>{
    const getConversaton = async ()=>{
      try{
       const res = await axios.get(`https://echosphere-5ixt.onrender.com/chat/${id}`)
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const  data  = await axios.get(`https://echosphere-5ixt.onrender.com/message/${id}`);
