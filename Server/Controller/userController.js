...
const loginUser = async (req, res) => {
  ...
  if (!user?.verified) {
    return res.status(404).json({
      status: "failed",
      message:
        "User email is not verified .Check your email account and verify your eamil",
    });
  }
  if (!user.password) {
    return res.status(400).json({
...
const updateUser = async (req, res) => {
  const { firstName, lastName, location, profession, photo, coverPhoto } =
    req.body.newData;
  ...
  const user = await UserSchema.findByIdAndUpdate(userId, updateData, {
    new: true,
  });
...
const getPost = async (req, res) => {
  ...
  const posts = await PostSchema.find(search ? searchPostQuery : {})
    .populate({
      path: "userId",
